...
async def get_message(chatroom_id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    # i want users name also not just id
    messages = db.query(models.Message).filter(models.Message.chatroom_id == chatroom_id).order_by(models.Message.created_at).all()
