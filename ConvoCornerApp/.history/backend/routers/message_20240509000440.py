...
@router.get("/{chatroom_id}", response_model=List[schemas.MessageOut])
async def get_message(chatroom_id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    
    messages = db.query(models.User.name, models.Message.text, models.Message.sent_at).join(models.Message, 
        models.User.id == models.Message.user_id).filter(models.Message.chatroom_id == chatroom_id).order_by(models.Message.sent_at)
