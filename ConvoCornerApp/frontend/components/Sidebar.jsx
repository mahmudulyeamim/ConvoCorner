...
export default function Sidebar({ activeChatRoom, setActiveChatRoom }) {
  ...
  useEffect(() => {
    setActiveChatRoom(active)
  } , [active])

  return (
    <div className='min-w-[22%] overflow-scroll min-h-screen sidebar border-r border-gray-200'>
      <h1 className='text-2xl font-bold text-center py-4 text-white'>Your Chat Rooms</h1>
      {chatRooms && chatRooms.length > 0 && chatRooms?.map((room, index) => (
