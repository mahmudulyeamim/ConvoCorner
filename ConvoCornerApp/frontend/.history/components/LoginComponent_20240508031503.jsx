...
export default function LoginComponent() {
    ...
    async function loginHandler() {
        setLoading(true)
        const url = `${process.env.NEXT_PUBLIC_ENDPOINT}/auth/login`
        const data = {
            username : username,
            password
        }
    ...
    <div className='w-full bg-[var(--bg-c1)] h-full flex items-center justify-center'>
            <div className='flex flex-col items-center justify-center p-8 rounded-md bg-white'>
                ...
                <div className='flex flex-col sm:my-2 w-full'>
                    <label  htmlFor="email" className='text-black text-lg 3xl:text-xl my-4'>UserName<span className='text-red-700 mx-1'>*</span></label>
                    <input type="text" value={username} placeholder="Enter Your UserName" className='border-2 border-gray-200 bg-white px-8 py-2 sm:py-4 w-full focus:border-[#227f7e] rounded-lg focus:outline-none' onChange={e => setUsername(e.target.value)} />
                </div>
